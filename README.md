...
- 🎓 **系统化学习路径**: 18个精心设计的课程，从基础语法到高级应用
- 🏋️ **实战练习导向**: 216个真实场景练习题，学以致用
...
- **第11课**: 进阶微积分 (导数定义、偏导、泰勒展开、变限积分)
- **第12课**: 概率统计与信息论 (贝叶斯、分布、期望方差、熵与KL散度)
- **第13课**: 线性代数进阶 (行列式、特征分解、SVD、Kronecker积)
- **第14课**: 物理与工程公式 (牛顿定律、麦克斯韦方程组、薛定谔方程)
- **第15课**: 复变函数与积分变换 (柯西-黎曼、留数、拉普拉斯与Z变换)
- **第16课**: 离散数学与算法 (大O、递推、生成函数、图论、布尔代数)
- **第17课**: 微分方程与偏微分方程 (特征方程、拉普拉斯/热/波动方程)
- **第18课**: 数学排版进阶 (aligned对齐、伸缩括号、\\text与\\operatorname)

### 🏋️ 练习中心
- **练习题库**: 216个精心设计的练习题，覆盖全部18课知识点
- **智能筛选**: 按课程、难度、完成状态筛选练习题
- **搜索功能**: 快速搜索特定题目内容
- **练习统计**: 详细的学习数据分析和进度可视化
- **独立练习模式**: 专门的练习执行环境，专注答题
- **离线练习**: 免注册即可练习整个题库，支持按课程、难度筛选与随机抽题
...
- `GET /api/practice/list` - 获取所有练习题列表（支持 `course`/`difficulty`/`language` 参数）
- `POST /api/practice/complete` - 标记练习题已完成
- `GET /api/practice/stats` - 获取用户练习统计

### 系统相关 ✅
- `GET /api/health` - 健康检查
- `GET /api/env-check` - 运行时环境变量快照（排查部署配置）
- `GET /api/init-db` - 生产环境首次初始化题库（需 `INIT_DB_SECRET`）
- `GET /api/reset-db` - 开发环境重建题库（需开启 debug）
